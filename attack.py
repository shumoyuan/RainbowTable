...
def bobs_custom_pwhash_attack(pc, char_list, length, pwlist_obj):
    ...
    for i, pwitem in enumerate(pwlist_obj):
        ...
        pwhash = pwitem['hash']
        iterated_hash = SHA256.new((salt + '---bob''s super secure custom pw hash v1').encode('utf-8')).digest()
        for j in range(0, 10000):
            iterated_hash = SHA256.new(iterated_hash).digest()
            addite = binascii.hexlify(iterated_hash[:6]).decode('utf-8')
        if (addite and pwhash)!='':
            attackhash = pwhash.replace(addite,'')
            #print(attackhash)
            attackhash1 = attackhash[:int(len(attackhash)/2)]
            #print(attackhash1)
            attackhash2 = attackhash[int(len(attackhash)/2):]
            #print(attackhash2)
            pw_gen = crypto.candidate_bf_generator(char_list, int(length/2))
            while pw_gen:
                try:
                    candidate_pw = next(pw_gen)
                    candidate_pwhash = crypto.truncated_sha2_hex_digest(candidate_pw+ '-p1',12)[:12]
                    #print(attackhash1)
                    #print(candidate_pwhash)
                    assert len(attackhash1) == len(candidate_pwhash), 'Hash lengths do not match.'
                    if attackhash1 == candidate_pwhash:
                        pwdpart1 = candidate_pw
                        #print(pwdpart1)
                except StopIteration:
                    break
            ...
            while pw_gen:
                try:
                    candidate_pw = next(pw_gen)
                    candidate_pwhash = crypto.truncated_sha2_hex_digest(candidate_pw+ '-p2',12)[:12]
                    assert len(attackhash2) == len(candidate_pwhash), 'Hash lengths do not match.'
                    if attackhash2 == candidate_pwhash:
                        pwdpart2 = candidate_pw
                    #print(pwdpart2)
                except StopIteration:
                    break
        ...
        if pwdpart1 != '' and pwdpart2 != '':
            password = pwdpart1+ pwdpart2
            results[i] = password
